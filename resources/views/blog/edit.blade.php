...
<div class="p-8 max-w-5xl mx-auto w-full">
    <div class="mb-8 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
            <h2 class="text-3xl font-headline font-extrabold text-primary tracking-tight mb-2">Edit Artikel</h2>
    ...
    <form id="blogForm" action="{{ route('blog.update', $article->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        <div class="bg-surface-container-lowest rounded-xl p-6 shadow-[0_8px_24px_rgba(25,47,63,0.04)] space-y-4">
            <div>
                <input name="title" value="{{ old('title', $article->title) }}" class="w-full bg-transparent border-none text-3xl font-headline font-bold text-primary placeholder:text-slate-400 focus:ring-0 p-0 @error('title') ring-2 ring-error/40 rounded @enderror" placeholder="Masukkan Judul Artikel..." type="text" required/>
                @error('title')<p class="text-error text-xs mt-1 font-body">{{ $message }}</p>@enderror
            </div>
            <div>
                <input name="subtitle" value="{{ old('subtitle', $article->subtitle) }}" class="w-full bg-transparent border-none text-lg font-body text-slate-600 placeholder:text-slate-400 focus:ring-0 p-0" placeholder="Ringkasan atau Subjudul (opsional)..." type="text"/>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                <div class="bg-surface-container-lowest rounded-xl p-6 shadow-[0_8px_24px_rgba(25,47,63,0.04)]">
                    ...
                    <p class="text-xs text-slate-500 font-body mb-3">Opsional. Unggah jika ingin mengganti thumbnail (JPEG, PNG, GIF, WebP, maks. 5 MB).</p>
                    <div id="upload-prompt" class="{{ $article->thumbnail ? 'hidden' : '' }}">
                    ...
                    <img id="preview-image" src="{{ $article->thumbnail ? asset('storage/' . $article->thumbnail) : '' }}" class="{{ $article->thumbnail ? '' : 'hidden' }} w-full h-auto rounded-lg object-cover cursor-pointer" alt="Pratinjau thumbnail">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var initialContent = @json(old('content', $article->content));
        if (initialContent) {
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                var hasExistingThumbnail = {{ $article->thumbnail ? 'true' : 'false' }};
                if (clickedButton === 'publish' && !hasExistingThumbnail && thumbnailInput.files.length === 0) {
                    e.preventDefault();
